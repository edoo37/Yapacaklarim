...
class RemoveDataFromStorageUsecase @Inject constructor(private val repository : TodoRepository) {
    suspend fun invoke(todoData: TodoData) = flow {
        try {
            emit(Resource.Loading())
            emit(Resource.Success(repository.removeTodoDatatoStorage(todoData)))
        }catch (t:Throwable){
